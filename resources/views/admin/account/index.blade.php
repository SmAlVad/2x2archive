...
@section('content')
  ...
  <table class="table table-striped">
    ...
    @forelse($accounts as $account)
      <tr @if($account->is_cancelled)style="opacity: .4;"@endif>
        <td>
        ...
        <td>И{{$account->number}}@if($account->is_cancelled)(аннулирован)@endif</td>
          ...
          @else
            @can('key-delete')
              <form action="{{ route('admin.accounts.activate', $account->id) }}" method="post">
                ...
                <button type="submit" class="btn btn-primary btn-sm" onclick="return confirm('Вы уверены?')">оплатить
                </button>
          ...
          @else
            @can('key-delete')
              <form action="{{ route('admin.accounts.cancelled', $account->id) }}" method="post">
                ...
                <button type="submit" class="btn btn-primary btn-sm" onclick="return confirm('Вы уверены?')">
                  аннулировать
                </button>
        ...
        <td class="text-center">
          <a href="{{ route('admin.accounts.print_acc', $account->id) }}" style="font-size: 1.2rem"
        ...
        <td class="text-center">
          @if($account->is_paid)
            <a href="{{ route('admin.accounts.print_act', $account->id) }}" style="font-size: 1.2rem"
               class="btn btn-secondary">
              <i class="fas fa-print"></i>
            </a>
          @endif
